...
from rug.api import get_deployments_by_name
from rug.geo import fetch_track_to_df, average_daily_track_gps, latlon_to_geojson_track
...
def main(args):
    ...
    deployment_names = args.deployment_names
    debug = args.debug
#    glider = args.glider
    daily = args.daily
...
#            return 1
    ...
    deployments = get_deployments_by_name(deployment_names)
...
#    deployments = locate_datasets(deployments, north=north, south=south, east=east, west=west)
    ...
    if debug:
        ...
        sys.stdout.write(
            '{:}\n'.format(tabulate.tabulate(deployments[print_columns], tablefmt=table_format, headers='keys')))
        logging.info('{:} matching data sets found'.format(deployments.shape[0]))
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('deployment_names',
                            nargs='+',
                            help='One or more registered RU-COOL deployment names',
                            type=str)

    arg_parser.add_argument('-t', '--template',
                            help='Specify the KML template to be used',
                            default=os.path.realpath(os.path.join(os.path.dirname(__file__), '../src/kml/templates/simple_tracks.kml')))
